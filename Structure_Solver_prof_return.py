...
def profiles(stellar_m, f_d):
        M = stellar_m*solarmass
        fdm = f_d
        Rguess   = 12.0**7*M/solarmass*4000.0
        # temperature finder
        ...
        while Not_Close:
                ...
                print("Enhancement: ", S*(v_disp/0.1)**2/1.549)
                print("v_disp: ", v_disp)
                print("alpha: ", alpha)
                print("c_param: ", c_param)
                sigma_v = S*3.0*10**(-32.0)*(v_disp/0.1)**2/1.549 # Sommerfeld enhancement and v-dependence of sigma_v combined together
                ...
                for i in range(rho_g.size):
                        photo[i] = Opacity.opac(np.log10(rho_g[i]),T[i])*P[i]-2.0/3.0*GN*M/r[i]**2
        ...
        if True :
                ...
                # print('G_ann  = ', Gamma_ann)
                #exit()
        return rho_d*rhoconvert, rho_g*rhoconvert, T, P, r/AU, photo
